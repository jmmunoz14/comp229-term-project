...
const SurveyList = () => {
  ...
  const surveysData = async () => {
    const res = await fetch(
      'https://surveymeanbackend.herokuapp.com/survey/list_attend',
    )
    ...
    let output = data.map((e) => {
      return {
        ...
        surveyTitle: e.title,
        surveyType: {
          agreeDisagree: e.type === 'Agree or Disagree' ? true : false,
          multipleChoice: e.type === 'Multiple Choice' ? true : false,
        },
      }
    })
